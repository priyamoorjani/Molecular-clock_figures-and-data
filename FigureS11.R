# Figure S11 Maximum Likelihood - HC
library(ape)
library(adephylo)
...
file <- read.table(paste(DIR,"data/FigureS11.txt", sep=""), header = TRUE,sep="\t")
title <- c("ancestral A/T","ancestral A/T", "ancestral CpG","ancestral CpG","ancestral non-CpG G/C","ancestral non-CpG G/C")

pdf(paste(DIR, "/results/FigureS11.pdf", sep=""),width=(6*3), height=(6*2))
